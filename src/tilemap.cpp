...
// Tilemap2P* Tilemap2P::getInstance() {
//     if (instance == nullptr) {
//         instance = new Tilemap2P();
//     }
//     return static_cast<Tilemap2P*>(instance);
// }
            ...
            for (const auto& object : layer["objects"]) {
                ...
                if (width == 0 && height == 0) {
                    if (object.contains("polygon")) {
                        ...
                        if (object.contains("type") && object["type"] == "player2") {
                            if (player2Loaded) {
                                player2Loaded = false;
                                continue;
                            }
                            player2Position = Vector2{x, y};
                            if (player2 != nullptr) {
                                player2->setPositionBody(b2Vec2{player2Position.x, player2Position.y});
                                player2->setInitialPosition(player2Position);
                                string fPath = filePath.substr(4,3);
...
void Tilemap2P::setPlayer2(const std::string name) {
    ...
    TextHelper::loadTexture("coin", "small" + name);
    player2->Init(b2Vec2{player2Position.x, player2Position.y});
    player2->setPositionBody(b2Vec2{player2Position.x, player2Position.y});
    player2->setInitialPosition(player2Position);
    string fPath = filePath.substr(4,3);
    player2->setCurrentMap(fPath);
